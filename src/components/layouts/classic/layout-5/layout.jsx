...
import getlayout5OutputSection from "../resume-output/layout5-output";


const ClassicalLayout5 = memo((props) => {
  const {
    watch
  } = useFormContext();
  ...
  const sectionData = getlayout5OutputSection(key_val)
  ...
  return (
    <div className="w-full max-w-full" ref={ref}>
      <LayoutUi sectionRefs={sectionRefs} key_val={key_val} pages={pages} layoutId={5} />
    </div>
  );
});
